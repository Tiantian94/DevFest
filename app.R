...
library(dplyr)
f = 
df = read.csv("./Data/Accounting_total.csv", stringsAsFactors = FALSE)
...
ui <- fluidPage(
  ...
  mainPanel(
    tabsetPanel(type = "tabs", 
                tabPanel("Classroom", verbatimTextOutput("classroom")),
...
server <- function(input, output, session) {
  ...
  output$table <- renderPrint({
    ...
    #filter(df,input$b == df[,"building"], input$r == df[,"room"])
  })
}
